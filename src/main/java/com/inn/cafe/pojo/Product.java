...
@NamedQuery(name = "Product.getProductById", query = "SELECT new com.inn.cafe.wrapper.ProductWrapper(p.id, p.name, p.category.id, p.category.name, p.description, p.price, p.status) FROM Product p WHERE p.id = :id")
